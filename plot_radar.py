...
plot_radar.py  —  Real-time PPI radar from PCI-9812 + Furuno FAR-2xx8
                  with dynamic sampling rate (1 – 20 MS/s)

━━  HARDWARE  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  All 4 channels sample simultaneously at one shared clock (max 20 MS/s).
  HD, TRIG, and VIDEO are all processed at the selected rate — no
  software decimation.  Changing the rate stops the DMA, rebuilds all
  rate-dependent state, and restarts (~1 s freeze on the display).

━━  RATE / RANGE TABLE  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Rate      Range res   Bins / 3 NM   DMA half-buf
  ────────  ─────────   ───────────   ────────────
  1  MS/s   149.9 m          41         ~  8 MB
  2  MS/s    75.0 m          80         ~ 16 MB
  5  MS/s    30.0 m         189         ~ 40 MB
  10 MS/s    15.0 m         374         ~ 80 MB
  20 MS/s     7.5 m         745         ~160 MB
...
Usage:  python plot_radar.py
        Move the slider at the bottom to change the sampling rate live.
"""
...
HALF_PERIOD_S    = 0.5        # target DMA half-buffer duration (s)
N_BEARINGS       = 512
PPI_SIZE         = 600
...
# Polar buffer pre-allocated at max-rate column count so rate changes never
# need a reallocation.
...
# ─────────────────────────────────────────────────────────────────────────────
# RateConfig  — all parameters derived from the selected sampling rate
# ─────────────────────────────────────────────────────────────────────────────

class RateConfig:
    """
    Built once per rate selection.  The old instance stays valid for any
    process_half() call that already holds a reference (GIL-safe swap).
    """

    def __init__(self, rate_hz: int):
        self.rate_hz       = int(rate_hz)
        self.range_res_m   = C / (2 * self.rate_hz)
        self.n_range_bins  = min(int(MAX_RANGE_M / self.range_res_m) + 4,
                                 MAX_RANGE_BINS)

        # Minimum inter-pulse gaps (samples at this rate)
        self.gap_hd   = int(0.50  * self.rate_hz)   # 0.5 s   between HD pulses
        self.gap_trig = int(5e-4  * self.rate_hz)   # 0.5 ms  between TRIG pulses

        # DMA buffer: half-period target, then round UP so scans-per-half is a
        # power-of-2.  The PCI-9812 DMA engine requires this alignment.
        scans_half = int(HALF_PERIOD_S * self.rate_hz)
        k          = max(9, math.ceil(math.log2(max(scans_half, 1))))  # 2^9 min
        self.half_count = (1 << k) * N_CH   # total uint16 in one half
        self.read_count = self.half_count * 2

        # Polar-to-Cartesian lookup tables (computed once per config)
        self._build_lookup()
    ...
    @property
    def rate_str(self):
        return f'{self.rate_hz // 1_000_000} MS/s'
...
class DASK:
    ...
    def AI_9812_Config(self, card, tm, ts, tsl, at, tl, pc):
        e = self._dll.AI_9812_Config(
            ctypes.c_int16(card), ctypes.c_uint16(tm), ctypes.c_uint16(ts),
            ctypes.c_uint16(tsl), ctypes.c_uint16(at), ctypes.c_uint16(tl),
            ctypes.c_uint32(pc))
        if e:
            raise RuntimeError(f'AI_9812_Config error={e}')
    ...
    def AI_AsyncDblBufferMode(self, card, enable):
        ...
        if e:
            raise RuntimeError(f'AI_AsyncDblBufferMode error={e}')
    ...
    def AI_ContScanChannels(self, card, ch, rng, buf_ptr, count, sr, synch):
        ...
        if e:
            raise RuntimeError(f'AI_ContScanChannels error={e}')
    ...
    def AI_AsyncClear(self, card):
        ...
        if e:
            raise RuntimeError(f'AI_AsyncClear error={e}')
        return cnt.value

    def Release_Card(self, card):
        e = self._dll.Release_Card(ctypes.c_int16(card))
        if e:
            raise RuntimeError(f'Release_Card error={e}')
...
class RadarProcessor:
    """
    Thread-safe polar accumulator.  Polar array is pre-allocated at the
    maximum column count so rate changes never need a reallocation.
    """
    ...
    def apply_new_rate(self, new_cfg: RateConfig):
        """Switch config; called from main thread while acquisition is stopped."""
        with self._lock:
            self._cfg = new_cfg
    ...
    def process_half(self, raw_uint16: np.ndarray):
        """
        Process one DMA half-buffer (all channels, full rate, no decimation).
        Takes a local snapshot of _cfg so a rate change mid-call is GIL-safe.
        """
        cfg     = self._cfg
        ...
        # ── HD pulse detection (full rate) ────────────────────────────────────
        for hd_i in _rising_edges(ch[CH_HD], THRESH_HD, cfg.gap_hd):
            hd_gs = self._gscan + hd_i
        ...
        # ── TRIG pulse detection (full rate) ──────────────────────────────────
        trig_edges = _rising_edges(ch[CH_TRIG], THRESH_TRIG, cfg.gap_trig)
        if len(trig_edges) > 1:
            half_s  = n_scans / cfg.rate_hz
            self.prf_hz = self.prf_hz * 0.85 + 0.15 * len(trig_edges) / max(half_s, 1e-9)
        ...
        for ti in trig_edges:
            if ti + nb >= n_scans:
                continue                       # radial would straddle chunk end
...
class AcquisitionThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            # Clear any acquisition state left by a previous run that crashed
            # without calling AI_AsyncClear — this is the most common cause of
            # AI_ContScanChannels returning error -15.
            try:
...
def build_display(init_rate_idx: int):
    ...
    rate_slider = Slider(
        ax=ax_sl, label='Sample rate',
        valmin=0, valmax=len(RATE_OPTIONS) - 1,
        valinit=init_rate_idx, valstep=1,
        color='#205020',
    )
    ...
    fig.text(0.50, 0.005,
             ...
             f'—  all channels (HD, TRIG, VIDEO) sampled at the selected rate',
             color='#2a4a2a', fontsize=6.5, ha='center')

    return fig, dict(ppi_im=ppi_im, sweep_ln=sweep_ln,
...
def make_updater(proc: RadarProcessor, acq_ref: list, arts: dict, pending: list):
    frame_ctr = [0]
    ...
    def update(_frame):
        ...
        if pending[0]:
            pending[0] = False
            new_hz = pending[1]
            arts['status_txt'].set_text(f'Restarting at {new_hz // 1_000_000} MS/s …')
            acq_ref[0].stop()
            ...
            print(f'\n[Rate] → {new_hz/1e6:.0f} MS/s  '
                  ...
                  f'| half-buf {new_cfg.half_count * 2 // 1_000_000} MB')
...
def main():
    ...
    print(f'  DMA half-buf  : {init_cfg.half_count // N_CH} scans  '
          f'({init_cfg.half_count * 2 // 1_000_000} MB total)')

    proc    = RadarProcessor(init_cfg)
    acq     = AcquisitionThread(proc, init_cfg)
    acq_ref = [acq]
    pending = [False, DEFAULT_RATE]
